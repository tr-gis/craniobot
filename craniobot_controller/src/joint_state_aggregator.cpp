...
//  Condition for multiturn, Select the joint with gearing as per location in joint_states array and add required gear ratio     
        
       if(i==0 || i==1 || i==2)
        {
		msg_.position[i] = msg->current_pos/4;
		ROS_ERROR("joint%d_current:%f",i+1,msg->current_pos);		
		ROS_ERROR("joint%d:%f",i+1,msg_.position[i]);
		}    
        else if(i==3)
        {
		msg_.position[i] = msg->current_pos/3;
		ROS_ERROR("joint4_current:%f",msg->current_pos);		
		ROS_ERROR("joint4:%f",msg_.position[i]);
		
		}
		
        else if(i==4)
        {
		msg_.position[i] = msg->current_pos/2;
		ROS_ERROR("joint5_current:%f",msg->current_pos);		
		ROS_ERROR("joint5:%f",msg_.position[i]);
		}
	   else if(i==5)
	   {
